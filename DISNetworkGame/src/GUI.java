...
//						playerMoved(0,-1,"up",player);
						outToServer.writeBytes("MOVE "+player.xpos+" "+player.ypos+" "+player.name + " up"+ '\n');
					} catch (IOException e) {
...
//						playerMoved(+1,0,"right",player);
	...
	public void setupPlayers(){
		...
		player = new Player("Ib",9,4,"up");
		players.add(player);
		fields[9][4].setGraphic(new ImageView(hero_up));

		Player harry = new Player("Kaj",14,15,"up");
		players.add(harry);
...
//		fields[15][2].setGraphic(new ImageView(hero_up));
	...
	public static void setUpMethod(){
		try {
			clientSocket = new Socket("10.10.131.21",6791);
